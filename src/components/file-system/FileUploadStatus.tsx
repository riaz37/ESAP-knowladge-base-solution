...
export const FileUploadStatus: React.FC<FileUploadStatusProps> = ({
  ...
  const bundleState = bundleStatus?.status || status || "pending";
  const totalFiles =
  ...
  const remainingFiles = bundleStatus?.remaining_files || 0;
  const tasks =
    bundleStatus?.individual_tasks || initialResponse?.task_ids || [];
